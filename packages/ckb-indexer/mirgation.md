...
Lumos is used for execution on server side, so we provide `@ckb-yadomis/indexer` and @ckb-`lumos/sql-indexer`. While bringing flexibility, it also increases the complexity of using it, such as you have to run the database locally, compile the code, etc.
...
Instead of calling `indexer.startForever()` in `native-indexer`, you just need to create an indexer instance in `@ckb-yadomis/ckb-indexer`, and we will turn it on for you automatically.
...
```diff
import {RPC} from '@ckb-yadomis/toolkit';
